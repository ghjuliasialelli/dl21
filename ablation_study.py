...
def main():
    # load options
    with open('options/ablation_study_options.json', 'r') as f:
        options = json.load(f)
...
def computeAbsoluteChangesInSample(data,new_data):
    ...
    for layer in data['model_weights']:
        ...
        original_layer = np.ravel(data['model_weights'][layer])
        manipulated_layer = np.ravel(new_data['model_weights'][layer])
        absoluteChange = np.abs(original_layer-manipulated_layer)
...
def visualizeChangeInSample(changesPerLayer,sampleNumber):
    ...
    fig = results.plot(kind="bar", title="absolut weight changes per layer in sample "+str(sampleNumber), figsize=(10,10))
...
def visualizeChanges(changesPerSample):
    ...
    for layer in sample:
        dictionary[layer] = np.reshape(sample[layer], (1,len(sample[layer])))

    
    for sample in changesPerSample[1:]:
        for layer in sample:
            transformed = np.reshape(sample[layer], (1,len(sample[layer])))
